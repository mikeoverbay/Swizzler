...
void main() {
    ...
    float t_alpha = clean_mask.a;

    if (clean_mask.r* clean_mask.a > 0.0) { mask_t.r = 1.0; }
    if (clean_mask.g* clean_mask.a > 0.0) { mask_t.g = 1.0; }
    if (clean_mask.b* clean_mask.a > 0.0) { mask_t.b = 1.0; }
    if (clean_mask.a* clean_mask.a > 0.0) { mask_t.a = 1.0; }
    if (clean_mask.a* clean_mask.a > 0.0) { mask_t.a = 1.0; }
        ...
        if (mask_function == 2) // mask
            {
                ...
                {
                tmix       = mix(vec4(0.0), vec4(01.0) ,  mask_t); // create invert mix
                color_mask = color_out * tmix; // tmix applies mask
                }else{
                color_mask = mix( vec4(0.0), color_out,  mask_t * clean_mask.a); // blend mode mask
                t_alpha    = 1.0 - t_alpha; // need to flip alpha for blending to work correctly
                }                
            }
